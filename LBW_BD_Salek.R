...
data <- read_dta("F:\\Machine learning  Based\\BDHS\\Cross country\\LBW\\Data\\BD_clean.dta")
...
p1 <- ggplot(boruta_df, aes(x = Feature, y = meanImp, fill = decision)) +
  ...
  coord_flip() +
  scale_fill_manual(values = c("Confirmed" = "#2a9d8f", 
                               "Tentative" = "#e9c46a", 
                               "Rejected" = "#f4a261")) +
  labs(title = "Boruta Feature Importance",
       y = "Mean Importance",
...
p2 <- ggplot(rfe_df[1:20, ], aes(x = Feature, y = Importance)) +
  geom_col(width = 0.7, fill = "#264653", color = "gray30") +
  coord_flip() +
  labs(title = "RFE Feature Importance",
       y = "Importance",
       x = NULL) +
...
# Ensure predictors are plain numeric data.frame
X_train_model <- as.data.frame(lapply(X_train_res, as.numeric))
X_test_model  <- as.data.frame(lapply(X_test_rfe, as.numeric))

# Response as factor (LBW is positive)
y_train_model <- factor(ifelse(y_train_res == 1, "LBW", "Normal"),
                        levels = c("Normal","LBW"))
y_test_model  <- factor(ifelse(y_test == 1, "LBW", "Normal"),
                        levels = c("Normal","LBW"))
...
for (name in names(model_list)) {
  ...
  set.seed(42)
  fit <- train(
    as.formula("lbw ~ ."),   # wrap formula to avoid method dispatch issues
    data = train_data,
    method = model_list[[name]],
    trControl = ctrl,
    metric = "ROC"
...
# Ensure predictors are numeric data.frames
X_train_model <- as.data.frame(lapply(X_train_res, as.numeric))
X_test_model  <- as.data.frame(lapply(X_test_rfe, as.numeric))

# Response as factor (LBW = positive)
y_train_model <- factor(ifelse(y_train_res == 1, "LBW", "Normal"),
                        levels = c("Normal","LBW"))
y_test_model  <- factor(ifelse(y_test == 1, "LBW", "Normal"),
                        levels = c("Normal","LBW"))
...
train_data <- cbind(X_train_model, lbw = y_train_model)
train_data$lbw <- factor(train_data$lbw, levels = c("Normal","LBW"))

# ---------------------------
# TrainControl for caret models (with tuning)
# ---------------------------
ctrl <- trainControl(
  method = "cv",
  number = 5,
...
# ---------------------------
# Define caret models with tuning grids
# ---------------------------
model_list <- list(
  "DecisionTree" = list(method = "rpart",
                        tuneGrid = expand.grid(cp = seq(0.001,0.05,0.01))),
  "RandomForest" = list(method = "rf",
                        tuneGrid = expand.grid(mtry = floor(sqrt(ncol(X_train_model))))),
  "LogisticRegression" = list(method = "glm", tuneGrid = NULL),
  "SVM" = list(method = "svmRadial",
               tuneGrid = expand.grid(C = 2^(-1:2), sigma = 0.01))
)

# ---------------------------
# Train caret models and compute metrics
# ---------------------------
results <- list()
...
for (name in names(model_list)) {
  ...
  fit <- caret::train(
    lbw ~ .,
    data = train_data,
    method = model_list[[name]]$method,
    tuneGrid = model_list[[name]]$tuneGrid,
    trControl = ctrl,
    metric = "ROC"
  )
  ...
  # Confusion Matrix
  cm <- caret::confusionMatrix(y_pred, y_test_model, positive = "LBW")
...
# ---------------------------
# XGBoost (xgb.train version with same metrics)
# ---------------------------
cat("\nTraining XGBoost...\n")
...
# ---------------------------
# XGBoost with manual tuning grid
# ---------------------------
...
# Prepare Data
# ---------------------------
X_train_model <- as.data.frame(lapply(X_train_res, function(x) as.numeric(as.character(x))))
X_test_model  <- as.data.frame(lapply(X_test_rfe, function(x) as.numeric(as.character(x))))
...
tune_grids <- list(
  DecisionTree = expand.grid(cp = seq(0.001, 0.05, by = 0.01)),
  RandomForest = expand.grid(mtry = floor(sqrt(ncol(X_train_model)))),
  LogisticRegression = NULL,   # glm has no tuning
  SVM = expand.grid(C = 2^(-1:2), sigma = 2^(-2:1))
)
...
# Save to CSV
# ---------------------------
output_path <- "F:/Machine learning  Based/BDHS/Cross country/LBW/R Programmings/Tables BD/LBW_Model_Results.csv"
...
#### ROC + Precision-Recall (Nature Medicine style) ####
library(pROC)
library(PRROC)
library(ggplot2)
library(gridExtra)
library(RColorBrewer)   # for polished bright palettes
...
roc_plot <- ggplot(roc_df, aes(x=FPR, y=TPR, color=Model)) +
  geom_line(size=1.5) +
  geom_abline(linetype="dashed", color="gray70", size=1) +
  scale_color_brewer(palette="Set2") +  # bright & journal-friendly
...
dt_plot <- ggplot(dt_imp, aes(x=reorder(Feature, Overall), y=Overall, fill=Overall)) +
  ...
  theme_minimal(base_size=14) +
  theme(
    plot.title = element_text(face="bold", size=16, hjust=0.5, color="#b30000"),
    axis.title = element_text(face="bold"),
    axis.text = element_text(color="#b30000"),
    legend.position = "none",
...
lr_plot <- ggplot(lr_imp, aes(x=reorder(Feature, Importance), y=Importance, fill=Importance)) +
  ...
  theme_minimal(base_size=14) +
  theme(
    plot.title = element_text(face="bold", size=16, hjust=0.5, color="#8c2d04"),
    axis.title = element_text(face="bold"),
    axis.text = element_text(color="#8c2d04"),
    legend.position = "none",
    panel.grid.major.y = element_line(color="#e0e0e0"),
    panel.grid.minor = element_blank()
  )
...
dt_plot <- ggplot(dt_imp, aes(x=reorder(Feature, Overall), y=Overall, fill=Overall)) +
  geom_col(color="black", width=0.7) +
  scale_fill_gradient(low="#dadaeb", high="#756bb1") +
  coord_flip() +
  labs(title="Decision Tree Feature Importance",
       x="Features", y="Importance") +
  theme_minimal(base_size=14) +
  theme(
    plot.title = element_text(face="bold", size=16, hjust=0.5, color="#4a1486"),
    axis.title = element_text(face="bold"),
    axis.text = element_text(color="#4a1486"),
    legend.position = "none",
    panel.grid.major.y = element_line(color="#e0e0e0"),
    panel.grid.minor = element_blank()
  )
...
lr_plot <- ggplot(lr_imp, aes(x=reorder(Feature, Importance), y=Importance, fill=Importance)) +
  geom_col(color="black", width=0.7) +
  scale_fill_gradient(low="#f2e6f9", high="#dd1c77") +
  coord_flip() +
  labs(title="Logistic Regression Feature Importance (|Coefficient|)",
       x="Features", y="Absolute Coefficient") +
  theme_minimal(base_size=14) +
  theme(
    plot.title = element_text(face="bold", size=16, hjust=0.5, color="#880085"),
    axis.title = element_text(face="bold"),
    axis.text = element_text(color="#880085"),
    legend.position = "none",
    panel.grid.major.y = element_line(color="#e0e0e0"),
    panel.grid.minor = element_blank()
  )
...
dt_plot <- ggplot(dt_imp, aes(x=reorder(Feature, Overall), y=Overall, fill=Overall)) +
  geom_col(color="black", width=0.7) +
  scale_fill_gradient(low="#a6cee3", high="#1f78b4") +  # cool blues
  coord_flip() +
  labs(title="Decision Tree Feature Importance",
       x="Features", y="Importance") +
  theme_minimal(base_size=14) +
  theme(
    plot.title = element_text(face="bold", size=16, hjust=0.5, color="#1f355c"),
    axis.title = element_text(face="bold"),
    axis.text = element_text(color="#1f355c"),
    legend.position = "none",
    panel.grid.major.y = element_line(color="#e0e0e0"),
    panel.grid.minor = element_blank()
  )
...
lr_plot <- ggplot(lr_imp, aes(x=reorder(Feature, Importance), y=Importance, fill=Importance)) +
  geom_col(color="black", width=0.7) +
  scale_fill_gradient(low="#b2df8a", high="#33a02c") +  # greens
  coord_flip() +
  labs(title="Logistic Regression Feature Importance (|Coefficient|)",
       x="Features", y="Absolute Coefficient") +
  theme_minimal(base_size=14) +
  theme(
    plot.title = element_text(face="bold", size=16, hjust=0.5, color="#2a4d1f"),
    axis.title = element_text(face="bold"),
    axis.text = element_text(color="#2a4d1f"),
    legend.position = "none",
    panel.grid.major.y = element_line(color="#e0e0e0"),
    panel.grid.minor = element_blank()
  )
...
#### Feature Importance (Nature Medicine style) ####
library(ggplot2)
library(dplyr)
library(gridExtra)
...
dt_plot <- ggplot(dt_imp, aes(x=reorder(Feature, Overall), y=Overall)) +
  geom_col(fill=brewer.pal(8, "Set2")[1], width=0.7) +  # polished bright color
  coord_flip() +
  labs(title="Decision Tree Feature Importance",
       x="Features", y="Importance") +
  theme_minimal(base_size=14) +
...
lr_plot <- ggplot(lr_imp, aes(x=reorder(Feature, Importance), y=Importance)) +
  geom_col(fill=brewer.pal(8, "Set2")[2], width=0.7) +
  coord_flip() +
  labs(title="Logistic Regression Feature Importance (|Coefficient|)",
       x="Features", y="Absolute Coefficient") +
  theme_minimal(base_size=14) +
