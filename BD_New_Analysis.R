...
output_path <- "E:\\f drives\\Machine learning based\\LBW\\Updated Analysis\\Bangladesh\\Table"
...
write.csv(
  ...
  file = file.path(
    output_path,
    "Bangladesh_Model_Performance_95CI.csv"
  ),
  row.names = FALSE
)
...
write.csv(
  ...
  file = file.path(
    output_path,
    "Bangladesh_Confusion_Matrix_Counts.csv"
  ),
  row.names = FALSE
)
...
word_file <- file.path(
  output_path,
  "Bangladesh_ML_APA_Performance_Tables.docx"
)
...
#### 90. SHAP AND LIME EXPLAINABILITY FOR BEST DECISION TREE MODEL ####
...
# Convert BOTH datasets to exactly the same base R class
# This fixes the fastshap class mismatch error
...
# Ensure test columns follow exactly the same order as training columns
...
# Number of predictors actually used
...
pred_wrapper <- function(
  ...
  # Make sure fastshap permutations are also data.frame
...
#### 94. CHECK BEST-MODEL PREDICTIONS ####
...
shap_values <- fastshap::explain(
  ...
  # Original non-SMOTE training data used as background
  X = X_shap_train,
  
  # Untouched test observations being explained
  newdata = X_shap_test,
...
# Convert to matrix for consistent shapviz handling
...
shap_beeswarm <- shapviz::sv_importance(
  ...
  ggplot2::labs(
    ...
    title =
      "SHAP Feature Effects for the Decision Tree Model",
    
    subtitle =
      "Feature contributions to low birth weight predictions in the untouched test set",
...
#### 97. POLISHED SHAP BEESWARM PLOT ####
...
#### 97. HIGH-IMPACT POLISHED SHAP BEESWARM PLOT ####
...
shap_beeswarm <- shapviz::sv_importance(
  ...
  # Bright high-impact feature-value gradient
...
shap_importance <- shapviz::sv_importance(
  ...
  ggplot2::labs(
    ...
    subtitle =
      "Decision Tree model evaluated on the untouched test set",
...
#### 98. SHAP BEESWARM AND GLOBAL IMPORTANCE SIDE BY SIDE ####
...
shap_beeswarm <- shapviz::sv_importance(
  ...
  ggplot2::scale_color_gradientn(
    ...
    values = scales::rescale(
      c(0.00, 0.15, 0.30, 0.50, 0.70, 0.85, 1.00)
    ),
    name = "Feature value",
    guide = ggplot2::guide_colorbar(
      title.position = "top",
...
shap_importance <- shapviz::sv_importance(
  ...
  ggplot2::labs(
    title = "B. Global SHAP Importance",
...
# ---------------------------
combined_shap_plot <- shap_beeswarm + shap_importance +
...
#### 99. SELECT HIGH-RISK TEST OBSERVATION ####

# Select the untouched test observation
# with the highest predicted probability of LBW
...
shap_waterfall <- shapviz::sv_waterfall(
  ...
  ggplot2::labs(
    ...
    title =
      "SHAP Waterfall Plot for a High-Risk LBW Prediction",
    
    subtitle = paste0(
      
      "Decision Tree model; predicted LBW probability = ",
...
#### 100. HIGH-IMPACT POLISHED SHAP WATERFALL PLOT ####
...
#### 101. PREPARE LIME EXPLAINER ####

# Methods required for caret::train models
...
#### 102. SELECT SAME UNTOUCHED TEST CASE FOR LIME ####
...
#### 103. GENERATE LIME LOCAL EXPLANATION ####
...
#### 104. LIME FEATURE CONTRIBUTION PLOT ####
...
lime_feature_plot <- lime::plot_features(
  ...
  ggplot2::labs(
    ...
    title =
      "LIME Feature Contributions for a High-Risk LBW Prediction",
...
#### 104. HIGH-IMPACT POLISHED LIME FEATURE CONTRIBUTION PLOT ####
...
#### 105. SAVE HIGH-RESOLUTION LIME PLOT ####

ggplot2::ggsave(
  
  filename = paste0(
    "E:\\f drives\\Machine learning based\\LBW\\Updated Analysis\\",
    "Bangladesh\\Figure\\LIME_DecisionTree.tiff"
  ),
...
cat("\nLIME rows:\n")
print(nrow(lime_explanation))

cat("\nLIME columns:\n")
print(names(lime_explanation))

cat("\nLIME class:\n")
print(class(lime_explanation))
